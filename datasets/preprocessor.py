...
def build_from_path(hparams, input_dirs, output_dir, n_jobs=12, tqdm=lambda x: x):
  ...
  Args:
    ...
    - output_dir: output directory of the preprocessed speech spectrograms (mel, linear, wav) dataset
  ...
  for input_dir in input_dirs:
    with open(os.path.join(input_dir, 'metadata.csv'), encoding='utf-8') as f:
      for line in f:
        ...
        text = parts[2]
        futures.append(executor.submit(partial(_process_utterance, output_dir, basename, wav_path, text, hparams)))

  return [future.result() for future in tqdm(futures) if future.result() is not None]


def _process_utterance(output_dir, basename, wav_path, text, hparams):
  """
  ...
  Args:
    - output_dir: the directory to write the mel spectograms into
    - basename: the basename to use in the spectogram filename
